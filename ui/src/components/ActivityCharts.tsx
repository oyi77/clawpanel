...
export function RunActivityChart({ runs }: { runs: HeartbeatRun[] }) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("dash_noRuns")}</p>;
...
export function PriorityChart({ issues }: { issues: { priority: string; createdAt: Date }[] }) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("dash_noIssuesChart")}</p>;
...
export function IssueStatusChart({ issues }: { issues: { status: string; createdAt: Date }[] }) {
  ...
  const statusLabels: Record<string, string> = {
    ...
    in_review: t("status_inReview"),
    done: t("status_done"),
    blocked: t("status_blocked"),
    cancelled: t("status_cancelled"),
    backlog: t("status_backlog"),
  };
